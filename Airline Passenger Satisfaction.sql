...
SELECT Customer_Type, COUNT(*) as Customer_Type_Count, COUNT(*)*100/CAST(SUM(COUNT(*)) OVER() as float) AS Customer_Type_Percent
FROM airline_passenger_satisfaction
...
SELECT Type_of_Travel, COUNT(*) as Type_of_Travel_Count, COUNT(*)*100/CAST(SUM(COUNT(*)) OVER () as float) AS Percent_of_Type_of_Travel
FROM airline_passenger_satisfaction
GROUP BY Type_of_Travel
ORDER BY Type_of_Travel

-- Count and Percent of Age Group 

-- Count and Percent of Customer and Travel Type
...
SELECT Type_of_Travel, Satisfaction, COUNT(*) as Type_of_Travel_Count, COUNT(*)*100/CAST(SUM(COUNT(*)) OVER (PARTITION BY Type_of_Travel) as float) as Percent_of_Type_of_Travel
FROM airline_passenger_satisfaction
...
SELECT Class, Satisfaction, COUNT(*) as Class_Count, COUNT(*)*100/CAST(SUM(COUNT(*)) OVER (PARTITION BY Class) as float) as Percent_of_Class
FROM airline_passenger_satisfaction
...
SELECT Gender, Satisfaction, COUNT(*) as Gender_Count, COUNT(*)*100/CAST(SUM(COUNT(*)) OVER (PARTITION BY Gender) as float) as Percent_of_Gender
FROM airline_passenger_satisfaction
...
SELECT Satisfaction,
    COUNT(*) AS Arrival_Delays_Count,
    AVG(CAST(Arrival_Delay AS decimal(10,2))) AS Avg_Arrival_Time_Delay
FROM airline_passenger_satisfaction
GROUP BY Satisfaction, (CASE WHEN Arrival_Delay > 0 THEN 1 ELSE NULL END)
...
-- Count and Percent of Buisness Passengers by Class
